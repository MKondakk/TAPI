import { Body, Controller, Get, Path, Post, Query, Route, Request, Response, Tags, SuccessResponse, Delete, Put, Example } from 'tsoa';
import * as express from 'express';
import { createPatient, deletePatient, getAllPatients, getPatientById, getPatientsByOwnerId, updatePatient } from '../services/patientService';
...
import { GET_ALL_PATIENTS_RESPONSE, PATIENT_RESPONSE } from '../examples/patient';
...
export class PatientController extends Controller {
    ...
    public async getPatients(@Request() req: express.Request, @Query() ownerId?: number): Promise<any> {
    ...
    public async getPatientById(@Path() id: number, @Request() req: express.Request): Promise<any> {
    ...
    @Response<InternalError>(500, 'Unexpected error')
    @Example<Patient>(PATIENT_RESPONSE)
    public async createPatient(@Body() requestBody: CreatePatientRequest, @Request() req: express.Request): Promise<any> {
    ...
    @Response<InternalError>(500, 'Unexpected error')
    @Example<Patient>(PATIENT_RESPONSE)
    public async updatePatient(@Path() id: number, @Body() requestBody: CreatePatientRequest, @Request() req: express.Request): Promise<any> {
    ...
    public async deletePatient(@Path() id: number, @Request() req: express.Request): Promise<any> {
